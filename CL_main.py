...
from utils.config import get_config
from trainers import CL_vificlip_pose_text_merged_model_AS_7Feb24 as vificlip 
...
def train_one_epoch(epoch, model, criterion, dist_loss_func, optimizer, lr_scheduler, train_loader, config, mixup_fn):
    ...
    for idx, batch_data in enumerate(train_loader):
        ...
        if config.TRAIN.LOSS_SCALE:
            ...
            total_loss = (1-alpha)*dist_loss + alpha*total_ce_loss # + alpha*text_pose_loss
        ...
        image_text_loss_meter.update(total_ce_loss.item(), len(label_id))
