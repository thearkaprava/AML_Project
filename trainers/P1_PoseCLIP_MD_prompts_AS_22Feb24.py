...
from Text_Prompt import get_text_prompt_embedding
...
class VLPromptLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model, logger):
        ...
        if ZS_evaluation:
            ...
            # breakpoint()
            tokenized_prompts = get_text_prompt_embedding()
            # prompt_merged_sent_list = [','.join(part) for part in prompt_sent_list]
            # # prompt_merged_sent_dict = {prompt.split(',')[0]: prompt for prompt in prompt_merged_sent_list}

            # prompts = [prompt_merged_sent for prompt_merged_sent in prompt_merged_sent_list]
            # # prompts = [prompt_sent_list(class_id) for class_id in classnames]
...
class ViFiCLIP(nn.Module):
    ...
    def forward(self, image, hyperformer_data=None): #, pose_embedding):
        ...
        if hyperformer_data is not None:
            ...
            text_pose_logits = pose_embedding @ text_features.t()
            

            # return image_text_logits, text_pose_logits
            return text_pose_logits
        else:
