...
class ClipTextEncoder2(nn.Module):
    def __init__(self, clip_model3):
        super().__init__()
        self.transformer = clip_model3.transformer
        self.positional_embedding = clip_model3.positional_embedding
        self.ln_final = clip_model3.ln_final
        self.text_projection = clip_model3.text_projection
        self.dtype = clip_model3.dtype
...
class ViFiCLIP(nn.Module):
    def __init__(self, cfg, classnames, clip_model, clip_model2, clip_model3, logger, args=None):
        super().__init__()
        ...
        self.clip_text_encoder2 = ClipTextEncoder2(clip_model3)
        ...
        if args is not None:
            ...
            self.hyperformer_model2 = self.load_model(args, cfg)
    ...
    def forward(self, image, hyperformer_data=None): #, pose_embedding):
        ...
        if hyperformer_data is not None:
            ...
            pose_embedding2 = self.hyperformer_model2(hyperformer_data)

            clip_text_features2 = self.clip_text_encoder2(prompts, tokenized_prompts)
            clip_text_features2 = clip_text_features2 / clip_text_features2.norm(dim=-1, keepdim=True)
            text_pose_logits2 = pose_embedding2 @ clip_text_features2.t()
        

            return image_text_logits, text_pose_logits, text_pose_logits2
...
def returnCLIP(config, logger=None,
    ...
    clip_model3 = load_clip_to_cpu(config)

    logger.info("Building ViFi-CLIP CLIP")
    model = ViFiCLIP(config, class_names, clip_model, clip_model2, clip_model3, logger, args)
    ...
    for name, param in model.named_parameters():
        ...
        if ('clip_text_encoder' in name and 'clip_text_encoder2' not in name) or ('hyperformer_model' in name and 'hyperformer_model2' not in name):
            param.requires_grad_(False)
